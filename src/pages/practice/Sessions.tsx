/**
 * Practice Sessions History — Real data from practice_sessions table
 */
...
import { ArrowLeft, Target, Dumbbell, Crosshair, Aperture } from 'lucide-react';

const DRILL_ICONS: Record<string, any> = {
  putting: Target,
  distance: Dumbbell,
  rangefinder: Crosshair,
};

const DRILL_LABELS: Record<string, string> = {
  putting: 'Putting Grid',
  distance: 'Distance Control',
  rangefinder: 'Rangefinder Sim',
};
    ...
    <AppLayout>
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="glass-card p-4 text-center">
            <p className="text-2xl font-black">{weekHours}h</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest">This Week</p>
          </div>
          <div className="glass-card p-4 text-center">
            <p className="text-2xl font-black">{totalMinutes}m</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Total Time</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {sessions.map((s, i) => {
              const Icon = DRILL_ICONS[s.drill_type] || Aperture;
              return (
                <div key={s.id} className="glass-card p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {new Date(s.created_at).toLocaleDateString()} · {Math.round((s.duration_seconds || 0) / 60)}m
